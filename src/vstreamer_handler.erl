...
page_handler(<<"list">>) ->
    ...
    VideoLinks = lists:concat([
        "<li><a href=\"/page/video/" ++ Video ++ "\">" ++ Video ++ "</a></li>" || Video <- vstreamer_videos:get_video_list()
    ]),
...
page_handler(<<"video/", VideoName/binary>>) ->
    ...
    case vstreamer_videos:is_exist_video(VideoName) of
        true ->
            ...
            {200, Header, vstreamer_pages:embed_data(File, "%%VIDEO_NAME%%", binary_to_list(VideoName))};
        false ->
